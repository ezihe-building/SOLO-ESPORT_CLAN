...
const GetUserParams = z.object({ id: z.number().int() });
...
const UpdateUserStatsBody = z.object({
  ...
  clanPoints: z.number().int().nullable().optional(),
  mvpCount: z.number().int().nullable().optional(),
  activity: z.number().int().nullable().optional(),
  tournamentWins: z.number().int().nullable().optional(),
  scrimWins: z.number().int().nullable().optional(),
});
...
router.get("/users/:id", async (req, res): Promise<void> => {
  ...
  const parsed = GetUserParams.safeParse({ id: parseInt(raw, 10) });
...
router.patch("/users/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  if (id !== sessionUserId && !isManagement(sessionRole)) {
